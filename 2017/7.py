...
def parse(data):
    ...
    for line in data.splitlines():
        if '->' in line:
            node_and_weight, outbound_str = line.split('->')
            outbound = [o.strip() for o in outbound_str.split(', ')]
        else:
            node_and_weight = line
            outbound = []
        node, weight = node_and_weight.split()
        nodes[node] = int(weight.strip('()'))
        edges[node] = outbound
    return nodes, edges

def find_root(edges):
    """finds the root of a directed acyclic graph"""
    candidates = set(edges.keys())
    for node, outbound in edges.items():
        for o in outbound:
            candidates.remove(o)
...
def find_weights(nodes, edges, root):
    weights = {}
    def decend(node):
        subtower_weight = sum(decend(n) for n in edges[node])
        weights[node] = nodes[node] + subtower_weight
        return weights[node]
    decend(root)
    return weights
...
def mismatch(items):
    """returns the index of the mismatching item, or None"""
    if len(items) < 2:
        return None
    ...
    for index, item in enumerate(items):
        counter.setdefault(item, []).append(index)
...
def balance(nodes, edges, root):
    weights = find_weights(nodes, edges, root)
    stack = [root]
    while stack:
        node = stack.pop()
        stack += edges[node]

        tower_weights = [weights[o] for o in edges[node]]
        idx = mismatch(tower_weights)
        if idx is not None:
            imbalanced = edges[node][idx]
            correct_weight = tower_weights[idx-1]
            diff = correct_weight - tower_weights[idx]
            print("{} weight {} but should be {}, it needs adjust from {} to {}".format(
                imbalanced, tower_weights[idx], correct_weight, nodes[imbalanced], nodes[imbalanced] + diff))

    return 0  # No imbalance found


nodes, edges = parse(ex)
root = find_root(edges)
print(balance(nodes, edges, root))
