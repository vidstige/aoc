...
def check(left, comp, right):
    ...
    if comp == "!=":
        return left != right
    raise ValueError("Unknown comparision {}".format(comp))
...
def cpu(data):
    ...
    for line in data.splitlines():
        reg, op, value, _, left, comp, right = line.split()
        value = int(value)
        if check(evaluate(left), comp, evaluate(right)):
